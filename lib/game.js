...
Game.prototype.addUfo = function(ctx) {
  ...
  let L = this.layout;
  let spawnPositions = L.ufoSpawnXs;
  ...
  if (spawnUfoChance < 1) {
    ...
    let ufoShip = new Ship ({
      ...
      img: ufoImage,
      radius: L.ufoRadius,
      pos: [spawnPositions[spawnIdx], L.ufoY],
      vel: vel,
      side: 'invader'
    });
...
Game.prototype.addInvaderShips = function(level = 1) {
  ...
  let y = L.invaderStartY;
  let invaderIdx = 0;
  let vel = [L.invaderBaseVelX + L.invaderLevelVelIncrement * level, 0];
  ...
  for (let row = 0; row < 3; row++) {
    ...
    for (let x = 1; x < 8; x++, invaderIdx++) {
      let invaderShip = new Ship ({
        ...
        radius: L.invaderRadius,
        pos: [
          x * L.invaderColSpacing + L.invaderColOffset,
          y
        ],
        vel: vel,
        side: 'invader'
...
Game.prototype.addShields = function() {
  let L = this.layout;
  for (let i = 0, x = L.shieldStartX; i < 5; i++, x += L.shieldStepX) {
    let shieldPosX = this.canvasSize[0] * x + L.shieldOffsetX;
    let shieldPosY = this.canvasSize[1] * L.shieldY;

    let shield = new Shield ({
      id: i,
      pos: [shieldPosX, shieldPosY],
      radius: L.shieldPieceSize,
      color: "#a0a09b",
      game: this
    });
